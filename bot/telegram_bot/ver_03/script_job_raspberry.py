...
def extract_jobs(tel_role: str, tel_profession: str, tel_area: str) -> None:
    """
    Парсер вакансий с сайта hh.ru
    :return: None
    """
    try:
        professional_role = f'&professional_role={tel_role}'  # специализация
        text_profession = f'&text={tel_profession}' if len(tel_profession) > 0 else ''
        area = tel_area  # регион
