...
# $> ./blocked_domain_rublacklist.sh 1 rutor.info h
...
if [ "$#" -lt 1 ] || [ "$#" -gt 3 ]; then
    echo '*** Необходимо передать имя ресурса ***'
    exit
fi
...
function info() {
  cat << END
  ***************************
  $> ./blocked_domain_rublacklist.sh rutracker.org - информация о заблокированном домене
  $> ./blocked_domain_rublacklist.sh 1 rutor.info - информация о всех заблокированных доменах, где есть ключевое слово
  $> ./blocked_domain_rublacklist.sh rutor.info h  - информация о заблокированном домене + help + ссылки
  ***************************
