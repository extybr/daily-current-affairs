#!/bin/bash
#################################################
# $> ./simple-parser-hh.sh 'инженер' 200000 5   #
#################################################
...
asu="&text=%D0%B8%D0%BD%D0%B6%D0%B5%D0%BD%D0%B5%D1%80%20%D0%B0%D1%81%D1%83"
...
if [ "$#" -gt 3 ]; then 
  echo -e "${WHITE}ожидалось не более 3 параметров, а передано $#${NORMAL}"
  exit 1
elif [ "$#" -eq 0 ]; then 
  salary="&only_with_salary=true&salary=250000"
elif [ "$#" -eq 1 ]; then
  if ! [ "$1" -ge 0 ] 2>/dev/null; then 
    echo -e "${WHITE}ожидалось число${NORMAL}"
    exit 1
  fi
  salary="&only_with_salary=true&salary=$1"
elif [ "$#" -eq 2 ]; then
  if ! [ "$2" -ge 0 ] 2>/dev/null; then 
    echo -e "${WHITE}2-ым параметром ожидалось число${NORMAL}"
    exit 1
  fi
  salary="&only_with_salary=true&salary=$2"
  if [ "$1" = "asu" ]; then 
    vacancy="${asu}"
  else enc=$("${SCRIPTS_DIRECTORY}/url_coder.sh" encoder "$1")
    vacancy=$(echo "&text=$enc" | sed "s/ /%20/g")
  fi
elif [ "$#" -eq 3 ]; then
  if ! [ "$2" -ge 0 ] 2>/dev/null || ! [ "$3" -ge 0 ] 2>/dev/null; then 
    echo -e "${WHITE}2-ым и 3-им параметром ожидались числа${NORMAL}"
    exit 1
  fi
  salary="&only_with_salary=true&salary=$2"
  period="&period=$3"
