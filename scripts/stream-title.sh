...
get_title_mpv() {
  timeout 5 mpv --no-video --length=1 --ytdl-format=worstaudio --network-timeout=3 "$1" 2>/dev/null \
...
if hash mpv 2>/dev/null; then
  title=$(get_title_mpv "$1")
elif hash ffmpeg 2>/dev/null; then
  title=$(get_title_ffmpeg "$1")
else echo -e '*** Ни одна из программ [\033[31mmpv, ffmpeg\033[0m] не найдена ***' && exit 1
fi

if [ -n "$title" ]; then
  echo -e "\033[36m$title\033[0m"
else echo -e '*** \033[31mНазвание трека на найдено\033[0m ***'
fi
